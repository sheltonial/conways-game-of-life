...
function removeDeadNeighbors(state) {
...
function addDeadNeighbors(state) {
...
function nextGeneration(coords) {
    ...
    const stateWithDeadNeighbors = addDeadNeighbors(state)
    let newState = {}

    for (let key in stateWithDeadNeighbors) {
        const {x, y} = keyToCoords(key)
    ...
    return stateToCoords(removeDeadNeighbors(newState))
}
